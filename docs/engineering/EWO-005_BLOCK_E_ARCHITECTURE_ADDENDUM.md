...
| ALTO    | Q-001 podía burlarse por el agotamiento de intentos: la colisión de folio, clasificada como recuperable, terminaba en `REJECTED (PROCESSING_FAILED)` — **una decisión fiscal tomada por omisión**                                                                                                                                               | *(Derogado por D-013)*                                                      |
...
**Regla explícita:** ninguna violación relacionada con `CfdiConcept`, `CfdiTax`, ni con el mismo `documentId`, produce jamás un rechazo fiscal — son señales de idempotencia o de carrera transitoria. *(Histórico: Antes de D-013, CFDI_DUPLICATE no podía emitirse automáticamente; ahora se emite de inmediato según la regla aprobada).*
...
> *(Histórico: Antes de la aprobación de D-013, aquí existía una salvaguarda (Q-001) para evitar que el agotamiento de intentos produjera un rechazo silencioso en caso de folio duplicado. Dado que D-013 ahora estipula el rechazo automático permanente por duplicado, la salvaguarda ya no es necesaria y ha sido derogada.)*
...
> *(Histórico: Esta sección ha sido **SUPERADA** por la decisión D-013. El comportamiento que describe queda **derogado**. Se conserva aquí únicamente para trazabilidad de la decisión.)*
>
...
| 76  | *(Histórico)* El caso de folio duplicado de otro documento se abordaba como recuperable antes de la decisión D-013. Ahora resulta en error permanente `REJECTED (CFDI_DUPLICATE)` inmediatamente.                                                                                                                                                                                                                                                                                | §9.3, AD-11                                                               |
...
| 83  | *(Histórico)* El criterio original sobre Q-001 queda derogado. Los rechazos automáticos por folio duplicado ahora sí ocurren, conforme a D-013.                                                                                                                                                                                                                                                                                                                                | §9.3                                                                      |
...
| G-16 | Agotamiento de intentos **causado por** folio duplicado                                | *(Derogado por D-013)*: El error por folio duplicado ahora es permanente y no entra en reintentos.                                |
...
| **Política de folio duplicado no definida** — el worker no puede rechazar automáticamente                                                                     | *(Resuelto)*                       | —          | *(Histórico)* Este riesgo fue resuelto por la decisión D-013, que aprueba el rechazo automático con `REJECTED (CFDI_DUPLICATE)`.                                                                                                                                                                                                                 |
...
- [ ] *(Derogado por D-013)*: Ningún camino —incluido el agotamiento de intentos— produce `REJECTED` automático por folio duplicado.
