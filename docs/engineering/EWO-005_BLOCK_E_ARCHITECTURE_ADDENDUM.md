...
| ALTO    | Q-001 podía burlarse por el agotamiento de intentos: la colisión de folio, clasificada como recuperable, terminaba en `REJECTED (PROCESSING_FAILED)` — **una decisión fiscal tomada por omisión**                                                                                                                                               | _(Derogado por D-013)_                                                      |
...
| Seed — permiso `cfdi.read`                                                          | Asignado a ADMINISTRADOR, CONTADOR, AUXILIAR, SUPERVISOR y AUDITOR (§12, actualizado por D-011)                              |
...
2. `P2002` sobre `companyId_folioFiscal` — el `folioFiscal` ya pertenece a **otro** documento: duplicado fiscal genuino, que produce rechazo permanente (D-013).
...
cuando esP2002(e) o esTransicionNoConfirmada(e):
  ...
  CASO F — dup != null  Y  dup.documentId != documentId
     ...
     → ✅ Q-001 RESUELTA por D-013: emitir REJECTED / CFDI_DUPLICATE / UnrecoverableError de inmediato
     → ERROR PERMANENTE + INCIDENTE + métrica dedicada
     → *(Histórico: antes de D-013, Q-001 obligaba a tratarlo como recuperable y aplicaba la salvaguarda de §10.2.3)*
...
**Regla explícita:** ninguna violación relacionada con `CfdiConcept`, `CfdiTax`, ni con el mismo `documentId`, produce jamás un rechazo fiscal — son señales de idempotencia o de carrera transitoria. _(Histórico: Antes de D-013, CFDI_DUPLICATE no podía emitirse automáticamente; ahora se emite de inmediato según la regla aprobada)._
...
| CFDI con `folioFiscal` duplicado — confirmado por evidencia positiva en el `catch` externo (otro documento con el mismo `folioFiscal`, AD-10.2) | `CFDI_DUPLICATE`           |
...
| Carrera entre intentos concurrentes (AD-10.2) | `P2002` en un `upsert` de hijo; `P2002` al hacer `create()` del `Cfdi` que el arbitraje por evidencia positiva no logra explicar.                                                                                                                       |
...
> _(Histórico: Antes de la aprobación de D-013, aquí existía una salvaguarda (Q-001) para evitar que el agotamiento de intentos produjera un rechazo silencioso en caso de folio duplicado. Dado que D-013 ahora estipula el rechazo automático permanente por duplicado, la salvaguarda ya no es necesaria y ha sido derogada.)_
...
**Estrategia de validación en dos fases (regla vinculante — corrige el hallazgo `ALTO` de la auditoría `FAILED` del 2026-08-01).** `E5-S3-T04` ejecuta **dos pasadas distintas y no intercambiables** sobre el mismo `xmlText`:

1. **Fase 1 — barrera sintáctica.** `XMLValidator.validate(xmlText)`, antes de cualquier parseo. Es la única responsable de garantizar la **buena formación**. Rechaza (verificado empíricamente contra la versión instalada, caso por caso): texto vacío, solo whitespace, tag sin cerrar, tags mal anidados, XML truncado, atributo sin cerrar, múltiples raíces y texto significativo fuera de la raíz. No impone límite de profundidad propio, de modo que no interfiere con `XML_MAX_DEPTH`.
2. **Fase 2 — parseo estructural.** `XMLParser.parse(xmlText)` de un solo argumento (nunca la sobrecarga deprecated de dos argumentos, que volvería a validar dentro del parseo) más el recorrido iterativo, responsables exclusivos de los límites de profundidad, nodos y atributos.

**Por qué la fase 1 es indispensable.** Verificado empíricamente contra `fast-xml-parser` `5.10.1`: `XMLParser.parse(xmlText)` de un solo argumento **no es un validador de buena formación** — acepta en silencio, sin lanzar, tags sin cerrar, tags mal anidados, XML truncado y texto significativo fuera de la raíz (este último se descarta del árbol sin dejar rastro). Confiar únicamente en el parseo permitiría que un CFDI truncado o con contenido descartado avanzara al extractor, **violando BR-XML-001**. La responsabilidad de la buena formación es de `E5-S3-T04` y **no se delega a `E5-S3-T05`**, que no la tiene asignada en ninguna de sus tarjetas.

**Doble pasada autorizada por seguridad, con su coste declarado.** Medido sobre un documento de 10 MB (el máximo que `XML_MAX_FILE_SIZE_BYTES` permite llegar hasta aquí): ~527 ms de validación frente a ~1036 ms de parseo, un sobrecoste aproximado del 50 % del parseo. Se acepta deliberadamente — la alternativa sería admitir documentos mal formados — y queda acotado por el límite de tamaño que `E5-S3-T03` ya impuso aguas arriba. El requisito previo de "parseo único" queda **explícitamente sustituido** por esta regla: no era compatible con el objetivo literal de `E5-S3-T04`.

**Deuda técnica declarada — `XMLValidator` deprecated.** `XMLValidator` está marcado `@deprecated` en `5.10.1`, con el mantenedor remitiendo al paquete separado `fast-xml-validator`. Se adopta igualmente porque es la **única** alternativa evaluada que cumple el objetivo literal de `E5-S3-T04` **sin agregar una dependencia nueva**: `fast-xml-validator@1.4.1` arrastraría tres paquetes transitivos adicionales (`detailed-xml-validator`, `path-expression-matcher`, `xml-naming`). Sigue presente y funcional en la versión instalada y fijada. **Ruta de migración registrada:** si una versión mayor futura de `fast-xml-parser` eliminara `XMLValidator`, la sustitución prevista es `fast-xml-validator` en versión exacta, en su propia tarea con su propia auditoría de dependencia.

**Clasificación del fallo sintáctico.** Todo fallo de la fase 1 se colapsa en `XML_SYNTAX_INVALID`. El `code` que devuelve el validador (`InvalidXml`, `InvalidTag`, `InvalidAttr`, `InvalidChar`) **no** separa de forma estable sintaxis de estructura — `InvalidXml` cubre por igual el texto vacío, un tag sin cerrar, múltiples raíces y texto sobrante al final —, y derivar `XML_STRUCTURE_INVALID` exigiría leer el `msg`, que **contiene contenido controlado por quien sube el archivo** (nombres literales de tag y atributo, verificado empíricamente) y cuya redacción no es estable entre versiones. Por eso se descartan por completo `msg`, `line` y `col`: de la respuesta del validador solo se usa el hecho binario de si pasó. `XML_STRUCTURE_INVALID` queda reservado a la invariante de raíz única posterior al parseo, que se conserva como defensa en profundidad.
...
### 5.3ter Detección de CFDI 4.0 (`E5-S3-T05`)

**Implementada en `E5-S3-T05`** (`apps/api/src/modules/xml-processing/cfdi-40-extractor.ts`) como **función pura** `detectCfdiVersion(parsedXml)`, sin NestJS, operando exclusivamente sobre el árbol `preserveOrder` ya devuelto por `validateXml` — sin volver a invocar `fast-xml-parser`, sin volver a validar sintaxis y sin volver a contar profundidad, nodos ni atributos.

**Alcance estrecho, decisión vinculante (`I-08`).** `E5-S3-T05` identifica la raíz, resuelve su namespace efectivo, confirma que es un `Comprobante` CFDI y lee `Version`. **No extrae ningún dato fiscal**: ni `UUID`, ni `RfcEmisor`/`RfcReceptor`, ni `SubTotal`/`Total`, ni `Moneda`, ni `Fecha`, ni `TipoDeComprobante`, ni conceptos, ni impuestos, ni el Timbre Fiscal Digital. Esos campos permanecen exclusivamente en `E5-S3-T06` (encabezado), `E5-S3-T07` (conceptos), `E5-S3-T08` (impuestos) y `E5-S3-T09` (ambiguos) — `E5-S3-T05` no las adelanta.

**Identidad por namespace URI, nunca por prefijo (hallazgo determinante, verificado empíricamente contra `fast-xml-parser` `5.10.1`).** El prefijo de un elemento XML es arbitrario y no forma parte de su identidad: un CFDI 4.0 legítimo puede declarar `xmlns:cfdi`, un prefijo alternativo (`xmlns:c`), o el namespace por defecto sin ningún prefijo (`xmlns=`) — los tres resuelven al mismo namespace y son igual de válidos. Un documento falsificado puede declarar `xmlns:cfdi` apuntando a un namespace inventado. Por eso `startsWith('cfdi:')` o cualquier comparación de texto sobre el nombre del tag **nunca** es un criterio de identidad válido: aceptaría el documento falsificado y rechazaría los CFDI legítimos con prefijo alternativo. La única fuente de verdad es el URI al que el prefijo resuelve, contra el ámbito de namespaces real del documento (`xmlns`/`xmlns:prefijo`), con herencia y reasignación por ámbito — igual que exige la especificación XML de namespaces. `E5-S3-T05` solo resuelve el ámbito de la raíz; expone `NamespaceScope` y las utilidades `extendNamespaceScope`/`resolveNamespacePrefix` para que `E5-S3-T06`–`T09` extiendan la resolución a sus propios elementos descendientes sin reimplementarla.

**Namespace de CFDI 3.3 reconocido deliberadamente (resuelve `I-12`).** AD-8 exige que un CFDI 3.3 real se rechace como `UNSUPPORTED_CFDI_VERSION`, no como `CFDI_STRUCTURE_INVALID` — es un CFDI real, solo de una versión no soportada, y un CFDI 3.3 real declara el namespace `http://www.sat.gob.mx/cfd/3`, no el de `cfd/4`. Por eso el conjunto de namespaces reconocidos como "familia CFDI" incluye ambos (`cfd/4` y `cfd/3`); un namespace fuera de ese conjunto —falso o ajeno— es `CFDI_STRUCTURE_INVALID`. Reconocer `cfd/3` no lo acepta: solo determina qué código de error produce el rechazo posterior por versión.

**Precedencia determinista.** La raíz (nombre local y namespace) se valida _antes_ que la versión. Un documento que no es un `Comprobante`, o cuyo namespace no es de la familia CFDI reconocida, se rechaza con `CFDI_STRUCTURE_INVALID` incluso si declara `Version="3.3"` o `Version="4.0"` — no tendría sentido describir un documento ajeno como "versión de CFDI no soportada".

**Comparación exacta de `Version`, nunca normalizada.** Se acepta únicamente el string `"4.0"`. `"4"`, `"4.00"`, `"4 .0"`, `"v4.0"` o cualquier otra variante de formato se tratan como una versión distinta — normalizar sería fijar un criterio fiscal sin fuente validada (`CLAUDE.md` regla 6). **Precisión empírica:** `Version=" 4.0 "` (espacios al inicio/final) no llega distinguible a `E5-S3-T05`, porque `trimValues: true` de `E5-S3-T04` (§5.2, inmutable) ya recorta también los valores de atributo, no solo el texto de los tags — no es una normalización que `E5-S3-T05` realice ni pueda evitar. Un espacio interior (`"4 .0"`) sí sobrevive intacto y sí se rechaza.

**Frontera de responsabilidad con Sprint 4 (regla vinculante, corrección `I-10`).** Igual que `E5-S3-T03`/`E5-S3-T04`, `E5-S3-T05` lanza únicamente un error tipado interno (`CfdiExtractionError`, con `code` discriminante: `CFDI_STRUCTURE_INVALID` | `UNSUPPORTED_CFDI_VERSION`). No traduce a `rejectionReason`, no ejecuta la Transacción C y no envuelve nada en `UnrecoverableError`. Esa clasificación externa pertenece al worker de Sprint 4 (AD-11). La redacción previa de la tarjeta ("Transacción C + `UnrecoverableError`") describía el mismo defecto documental ya identificado y corregido en `H-T03` y `H-T04-01` — queda **sustituida**.
...
Esta corrección modificó artefactos ya auditados de Sprint 1 y Sprint 2, por lo que quedó **`READY_FOR_AUDIT`** hasta su cierre por auditoría final independiente `READ ONLY` el 2026-08-03 (Codex, HEAD `6934b26f6f568c27d697f8c3f31cebaddb0261a0`, sin hallazgos nuevos: [`D-009_FINAL_AUDIT.md`](audits/D-009_FINAL_AUDIT.md)). **D-009 = `PASSED`.** `I-14` e `I-15` quedan **`RESOLVED`**. `E5-S3-T06` queda **desbloqueada**, permanece **no iniciada**.
...
BullMQ dispara Job { jobId, documentId, companyId }
    ...
    │                 CASO F  folio de OTRO documento → ✅ D-013 Aprobada: ERROR PERMANENTE +
    │                                                    incidente, emite REJECTED (CFDI_DUPLICATE)
...
@OnWorkerEvent('failed') — SE DISPARA EN CADA INTENTO FALLIDO, no solo al agotarse (AD-4.1):
    ...
    Si esTerminal:
      → *(Histórico: antes de D-013, aquí se ejecutaba la SALVAGUARDA Q-001 PREVIA de §10.2.3)*
      → Transacción C condicional (WHERE Document.status='PROCESSING')
        con rejectionReason extraído del UnrecoverableError (o PROCESSING_FAILED si fue por agotamiento)
...
| 3   | `Job = PROCESSING`, `Document = PROCESSING`  | **AUSENTE** (§10.1)                 | **No**     | `Job = FAILED` + `Document = REJECTED (PROCESSING_FAILED)`                                                                                                                           |
...
| 5   | `Job = PROCESSING`, `Document = PROCESSING`  | **`failed` (terminal retenido)**    | **No**     | **Leer el terminal retenido sin esperar a la retención.** `Job = FAILED` + `Document = REJECTED (PROCESSING_FAILED)`                                                                 |
...
| 9   | `Job = FAILED`, `Document = PROCESSING`      | cualquiera                          | **No**     | `Document = REJECTED (PROCESSING_FAILED)`                                                                                                                                            |
...
> _(Histórico: Esta sección ha sido **SUPERADA** por la decisión D-013. El comportamiento que describe queda **derogado**. Se conserva aquí únicamente para trazabilidad de la decisión.)_
>
> _Mientras Q-001 siga pendiente, una colisión de `folioFiscal` con **otro** documento se clasifica como **error recuperable** (AD-10.2 caso F). Esa clasificación tiene una consecuencia que debe cerrarse explícitamente:_
>
> _(...)_
>
> _**Coste asumido y documentado:** los documentos afectados quedan detenidos en `PROCESSING` hasta que Q-001 se resuelva. Es deliberado — es preferible un documento visiblemente detenido a un rechazo fiscal no aprobado (`CLAUDE.md` regla 6). El riesgo está registrado como **R-005** en `brain/RISKS.md`._
...
> **Actualizada el 2026-08-06 para reflejar `D-011`** (`brain/DECISIONS.md`, `EWO-SEC-NAV-001` `T03`). La tabla original de este addendum (2026-07-24) excluía a `SUPERVISOR` y `AUDITOR` de `cfdi.read`; `D-011` revisó ese criterio y les concede lectura estrictamente de solo lectura. La matriz canónica vigente por acción, con la que esta tabla debe permanecer sincronizada, es `BR-PERM-004` en `docs/04_BUSINESS_RULES.md`; ante cualquier divergencia futura, `BR-PERM-004` prevalece sobre esta tabla. Esta corrección es exclusivamente documental — no modifica la arquitectura de concurrencia del worker (`D-007`, AD-1 a AD-12) ni ningún otro contenido de este addendum.

**Decisión:** `cfdi.read` está agregado al `PERMISSION_CATALOG` del seed (`packages/database/prisma/permissions-catalog.ts`). Los roles con acceso son:

| Rol             | `document.read` | `cfdi.read`    | `document.download` | Razonamiento                                                                                                                                                                                 |
| --------------- | --------------- | -------------- | ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ADMINISTRADOR` | Sí              | **Sí**         | **Sí**              | Obtiene todos los permisos del catálogo automáticamente (`PERMISSION_CATALOG.map(p => p.key)` — línea del seed)                                                                              |
| `CONTADOR`      | Sí              | **Sí**         | **Sí**              | Requiere datos fiscales para contabilidad                                                                                                                                                    |
| `AUXILIAR`      | Sí              | **Sí**         | **Sí**              | Requiere datos fiscales para captura                                                                                                                                                         |
| `SUPERVISOR`    | Sí              | **Sí** (D-011) | **Sí**              | Revisa y aprueba casos sensibles; `docs/04` §5.1 le concede acceso a evidencia — lectura estrictamente de solo lectura, nunca `cfdi.generate`/`cfdi.cancel`                                  |
| `AUDITOR`       | Sí              | **Sí** (D-011) | **Sí**              | Rol de auditoría con acceso de solo lectura; el CFDI es el soporte legal del asiento contable que ya puede leer — lectura estrictamente de solo lectura, nunca `cfdi.generate`/`cfdi.cancel` |

**Aclaraciones:**

- `document.read` no implica `cfdi.read`, y ninguno de los dos implica `document.download`. Son tres permisos distintos con propósitos distintos.
- Acceder al archivo XML original (descarga) y acceder a los datos fiscales extraídos (CFDI) son operaciones con diferente sensibilidad fiscal, gobernadas por claves distintas: **la descarga del XML original depende de `document.download`, nunca de `cfdi.read`** — `cfdi.read` autoriza únicamente listar, ver resumen y ver datos fiscales estructurados (D-011, contrato vinculante puntos 6–7 y 10). Ver `BR-PERM-004` para la matriz completa por recurso/acción.
- `SUPERVISOR` y `AUDITOR` reciben `cfdi.read` en modo estrictamente lectura: nunca `cfdi.generate` ni `cfdi.cancel` (D-011, contrato vinculante puntos 6–7).
- Platform Admin (`isPlatformAdmin`) **no forma parte de esta matriz**: no es un valor de `RoleName`, no puede aparecer en `ROLE_PERMISSIONS` por construcción del tipo, y no hereda ninguno de estos permisos de forma implícita (`D-010`).
- El plan original §4.4 especificaba `cfdi.read` solo para CONTADOR y AUXILIAR. El seed lo amplió primero a ADMINISTRADOR (coherente con el patrón general del catálogo, donde ADMINISTRADOR recibe todos los permisos) y después, por `D-011`, a SUPERVISOR y AUDITOR.
- En el seed actual, `ADMINISTRADOR` ya recibe todos los permisos explícitamente — no requiere cambio en la lógica del seed.
...
| 20  | Un CFDI cuyo `folioFiscal` ya pertenece a **otro** documento se detecta por evidencia positiva y produce `REJECTED (CFDI_DUPLICATE)` automático mediante un error permanente, sin reintentos. En ningún caso se crean duplicados en `cfdi`.                                                                                                                                                                                                                                      | AD-3, §9.3                                                                |
| 21  | Un documento que agota sus `JOBS_ATTEMPTS` intentos **totales** (1 inicial + N−1 reintentos) por error recuperable termina en `REJECTED (PROCESSING_FAILED)`, no queda en `PROCESSING`                                                                                                                                                                                                                                                                                           | AD-4, AD-11                                                               |
...
| 28  | API-0027 y API-0028 devuelven 200 para ADMINISTRADOR, CONTADOR, AUXILIAR, SUPERVISOR y AUDITOR (todos con `cfdi.read`, D-011); devuelven 403 únicamente para ESTUDIANTE (sin entrada en `ROLE_PERMISSIONS`)                                                                                                                                                                                                                                                                      | §12, D-011                                                                |
...
| 34  | `cfdi.read` existe en la tabla `permissions` y está asignado a ADMINISTRADOR, CONTADOR, AUXILIAR, SUPERVISOR y AUDITOR (D-011) — estrictamente de lectura, nunca junto con `cfdi.generate`/`cfdi.cancel` para SUPERVISOR/AUDITOR                                                                                                                                                                                                                                                 | §12, D-011                                                                |
...
| 49  | Un `P2002` al crear el `Cfdi` se resuelve re-consultando el estado **en el `catch` externo, fuera de la transacción abortada**, sin leer `error.meta.target`; el duplicado por folio de otro documento se confirma con evidencia positiva y produce `REJECTED (CFDI_DUPLICATE)` automáticamente.                                                                                                                                                                                 | AD-10.2                                                                   |
...
| 76  | _(Histórico)_ El caso de folio duplicado de otro documento se abordaba como recuperable antes de la decisión D-013. Ahora resulta en error permanente `REJECTED (CFDI_DUPLICATE)` inmediatamente.                                                                                                                                                                                                                                                                                | §9.3, AD-11                                                               |
...
| 83  | _(Histórico)_ El criterio original sobre Q-001 queda derogado. Los rechazos automáticos por folio duplicado ahora sí ocurren, conforme a D-013.                                                                                                                                                                                                                                                                                                                                  | §9.3                                                                      |
...
| Deduplicación: segundo upload de un `folioFiscal` que ya pertenece a **otro** documento → el `Document` rechaza automáticamente con `REJECTED (CFDI_DUPLICATE)` por D-013, sin agotar intentos (gate G-15).                                                                                                                                                                                                                                                      | PostgreSQL + MinIO               |
...
| Job `PROCESSING` ausente de Redis **sin** `Cfdi` → `REJECTED (PROCESSING_FAILED)`                                                                                                                                                                                                                                                                                                                                                                                | PostgreSQL + Redis               |
...
| G-15 | Folio de **otro** documento                                                            | CASO F: error permanente + incidente; **REJECTED (CFDI_DUPLICATE)** inmediato (D-013)                                     |
| G-16 | Agotamiento de intentos **causado por** folio duplicado                                | _(Derogado por D-013)_: El error por folio duplicado ahora es permanente y no entra en reintentos.                        |
...
| **Política de folio duplicado no definida** — el worker no puede rechazar automáticamente                                                                     | _(Resuelto)_                       | —          | _(Histórico)_ Este riesgo fue resuelto por la decisión D-013, que aprueba el rechazo automático con `REJECTED (CFDI_DUPLICATE)`.                                                                                                                                                                                                                 |
...
- [ ] _(Derogado por D-013)_: Ningún camino —incluido el agotamiento de intentos— produce `REJECTED` automático por folio duplicado.
...
- [ ] API-0026 implementada en `DocumentsController`; requiere `document.download` (no `document.read` — clave independiente de descarga de binario, D-011; ver `docs/08_API_DESIGN.md` §9.5 nota `†`); restringida a estados `PROCESSING/PROCESSED/REJECTED`.
- [ ] Permiso `cfdi.read` en seed, asignado automáticamente a ADMINISTRADOR (via catálogo) y explícitamente a CONTADOR, AUXILIAR, SUPERVISOR y AUDITOR (D-011, estrictamente de lectura).
...
- [x] **Business rule de `folioFiscal` duplicado aprobada por el responsable de producto** (§9.3) e implementada conforme a lo aprobado (D-013).
