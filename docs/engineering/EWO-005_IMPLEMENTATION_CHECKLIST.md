...
- **Estado inicial:** `BLOCKED`.
- **Estado actual:** `READY_FOR_AUDIT` (implementado 2026-07-31).
- **Implementación (2026-07-31):** agregado `fast-xml-parser@^4.5.7` a `apps/api/package.json` (orden alfabético, entre `cookie-parser` y `helmet`) y actualizado `pnpm-lock.yaml` vía `pnpm install --filter @contaia/api...`. Versión elegida: dist-tag `legacy` del registro (`4.5.7`), no `latest` (`5.10.1`) — la línea `5.x` introduce cinco dependencias nuevas (`@nodable/entities`, `fast-xml-builder`, `is-unsafe`, `path-expression-matcher`, `xml-naming`) publicadas hace 2 semanas, mientras que `4.5.7` mantiene una sola dependencia (`strnum@^1.0.5`), historial largo (198 versiones totales del paquete) y es la línea que el propio mantenedor (`NaturalIntelligence/fast-xml-parser`) sigue publicando como `legacy` estable — preferida por menor superficie de cadena de suministro para un parser fiscal crítico (CFDI), sin necesidad todavía verificada de ninguna característica exclusiva de `5.x`. Ninguna dependencia adicional innecesaria: solo se añadió `strnum@1.1.2` (dependencia transitiva única de `fast-xml-parser`). Sin `engines` declarado por el paquete; compatible con Node `>=20` del monorepo y con NestJS `10.4.15`. No se agregó configuración, providers, servicios ni lógica de parseo — eso pertenece a `E5-S3-T03`/`E5-S3-T04`. Validaciones ejecutadas: `pnpm install` ✅, `typecheck` ✅, `eslint --max-warnings 0` ✅, `prettier --check` ✅, `nest build` ✅ (exit 0). Diff de `pnpm-lock.yaml` acotado exclusivamente a `fast-xml-parser@4.5.7` y su única dependencia `strnum@1.1.2` (verificado línea por línea; sin cambios a resoluciones de ningún otro paquete). Pendiente: auditoría independiente `READ ONLY` de Codex antes de marcar `PASSED`.
...
- **Sprint 3** — **`IN_PROGRESS`.** `E5-S3-T01` → **`PASSED`** el 2026-07-31 (cierre administrativo tras auditoría final independiente `READ ONLY`, Codex, sin hallazgos: `docs/engineering/audits/E5-S3-T01_FINAL_AUDIT.md`): creado `apps/api/src/modules/xml-processing/xml-processing.module.ts` — módulo NestJS vacío (`imports: []`, `providers: []`, `exports: []`), sin `forwardRef()`, sin dependencias con `JobsModule`/`DocumentsModule`/`StorageModule`/`@contaia/database`/`bullmq`/`fast-xml-parser`. `E5-S3-T02` (instalación de `fast-xml-parser@4.5.7`, dist-tag `legacy`, sin configuración ni lógica de parseo) implementada el mismo día, `READY_FOR_AUDIT` — pendiente de auditoría independiente `READ ONLY` con Codex antes de `PASSED`; ver tarjeta en sección 10. `E5-S3-T03`–`T12` permanecen `BLOCKED`. _(Estado histórico previo, mismo día: `READY_FOR_AUDIT`, pendiente de auditoría `READ ONLY` de Codex — superado por el veredicto `PASSED` registrado arriba. Antes: `NOT_STARTED`.)_
