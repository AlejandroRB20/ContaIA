...
- **Criterio de aceptación:** ningún documento llega al parser sin haber superado los cinco controles, y cada fallo produce un **error tipado interno** (`XmlPreValidationError` con `code`), nunca un parseo con opciones inseguras. _(Corregido el 2026-07-31 — hallazgo `I-03` del análisis técnico previo. La redacción anterior exigía que esta tarea produjera `XML_INVALID` "vía Transacción C + `UnrecoverableError`": ninguna de las tres cosas es alcanzable desde `E5-S3-T03`, porque la Transacción C y la clasificación `UnrecoverableError` pertenecen al worker de Sprint 4 (AD-11). Aplicado literalmente, ese criterio habría hecho imposible aprobar la tarea. La traducción `code` → `XML_INVALID`, la Transacción C y `UnrecoverableError` son responsabilidad de Sprint 4 — ver Addendum §5.3, "Frontera de responsabilidad con Sprint 4".)_
- **Pruebas requeridas:** suite unitaria propia en `xml-pre-validation.spec.ts` (ver Addendum §5.4, reparto de pruebas por capa); las de integración de extremo a extremo, en `E5-S3-T12`.
...
- **Estado inicial:** `BLOCKED`.
- **Estado actual:** `PASSED` (cierre administrativo 2026-08-01, HEAD auditado `79ce98489ae9300193d1380480278840f3563a1f`, auditoría final independiente `READ ONLY`: `docs/engineering/audits/E5-S3-T03_FINAL_AUDIT.md`). _(Estado histórico previo: `READY_FOR_AUDIT` (implementado 2026-07-31) — superado por el veredicto `PASSED`. Antes, mismo día 2026-07-31: `HABILITADA` / `NO INICIADA`.)_
- **Implementación (2026-07-31):** creados `apps/api/src/modules/xml-processing/xml-pre-validation.ts` (función pura `preValidateXmlBuffer(buffer, limits)`), `xml-pre-validation.errors.ts` (clase única `XmlPreValidationError` con `code` discriminante — mismo patrón ya auditado de `StorageError`) y `xml-pre-validation.spec.ts` (64 pruebas). **Función pura, no provider**: sin `@Injectable`, sin `Logger`, sin `console`, sin estado; `xml-processing.module.ts` **no fue modificado** y sigue vacío. **Cero imports de `fast-xml-parser`** — el único import de producción es el módulo local de errores. Límites inyectados como parámetro (`XmlSecurityLimits.maxFileSizeBytes`): no se creó `xmlEnvSchema` ni variable de entorno alguna, porque `XML_MAX_FILE_SIZE_BYTES` (Addendum §10.3) sigue sin existir en `@contaia/config` y su creación pertenece a otra tarea. **Orden de controles** (parte del contrato de seguridad): precondiciones de programación → vacío → tamaño → BOM sobre bytes crudos → `NUL` sobre bytes crudos → decodificación UTF-8 estricta → declaración de encoding → `DOCTYPE`/`ENTITY` → forma mínima. **Decisiones técnicas verificadas:** (a) `TextDecoder('utf-8', { fatal: true, ignoreBOM: true })` en lugar de `Buffer.toString('utf8')`, que sustituye en silencio bytes inválidos por `U+FFFD` (falla abierta); (b) escaneo de byte `NUL` **antes** de decodificar, indispensable porque `U+0000` es UTF-8 válido y un UTF-16LE sin BOM atraviesa un decodificador estricto sin error — hay prueba dedicada de ese bypass; (c) BOM UTF-32 comprobado antes que UTF-16, porque `FF FE` es prefijo de `FF FE 00 00`; (d) el Buffer original se devuelve por referencia y nunca se muta (`subarray`, no copia), preservando los bytes para el SHA-256 de §7 paso 6; (e) límite **inclusivo**; (f) un límite no entero, ≤ 0, `NaN` o `Infinity` lanza `RangeError`, nunca un código de documento — comprobado antes de usarlo, porque `byteLength > NaN` es `false` y desactivaría el control en silencio. **Errores:** `XML_EMPTY`, `XML_TOO_LARGE`, `XML_ENCODING_UNSUPPORTED`, `XML_INVALID_BYTES`, `XML_DOCTYPE_FORBIDDEN`, `XML_ENTITY_DECLARATION_FORBIDDEN`, `XML_FORMAT_INVALID`; mensajes constantes por `code`, sin interpolar documento, Buffer, tamaños, el valor de un `encoding` declarado ni datos fiscales, y sin envolver el error nativo de decodificación. **Precedencia determinista** probada: tamaño → BOM → `DOCTYPE` → `ENTITY`. **Cobertura dirigida:** `xml-pre-validation.ts` 100 / 95.83 / 100 / 100 y `xml-pre-validation.errors.ts` 100 / 100 / 100 / 100 (statements/branches/functions/lines) — la única rama no cubierta es un `?? ''` inalcanzable que `noUncheckedIndexedAccess` exige tras una expresión regular que ya casó, documentado en el código. **Regresión completa:** 38 suites / 470 pruebas `PASSED`. `E5-S3-T04` **no fue iniciada**: no existe `xml-validation.service.ts` ni ningún otro archivo suyo. Pendiente: auditoría independiente `READ ONLY` de Codex antes de marcar `PASSED`.
...
- **Criterio de aceptación:** cada control excedido lanza un `XmlValidationError` interno con `code` discriminante (`XML_SYNTAX_INVALID`, `XML_STRUCTURE_INVALID`, `XML_DEPTH_EXCEEDED`, `XML_NODE_LIMIT_EXCEEDED`, `XML_ATTRIBUTE_LIMIT_EXCEEDED`, `XML_DANGEROUS_PROPERTY`). _(Corrección `I-04`, 2026-08-01: la redacción anterior — "produce `XML_INVALID` vía Transacción C + `UnrecoverableError`" — exigía tres efectos inalcanzables desde una función pura, el mismo defecto documental ya identificado y resuelto en `E5-S3-T03` como `H-T03`. La traducción a `XML_INVALID`/Transacción C/`UnrecoverableError` sigue siendo, sin excepción, responsabilidad exclusiva del worker de Sprint 4 — AD-11, Addendum §5.3bis.)_
...
- **Estado inicial:** `BLOCKED`.
- **Estado actual:** `PASSED` (cierre administrativo 2026-08-01, HEAD auditado `66703145866349bc362c1f6a1c7bd8cf96ca3466`, reauditoría final independiente `READ ONLY`: `docs/engineering/audits/E5-S3-T04_FINAL_AUDIT.md`, hallazgo `H-T04-01` `RESOLVED`, sin hallazgos nuevos). _(Estado histórico previo, mismo día: `READY_FOR_AUDIT` tras corregir el hallazgo `ALTO` — superado por el veredicto `PASSED`. Antes: `READY_FOR_AUDIT` con auditoría `FAILED` pendiente de corrección. Antes: `HABILITADA` / `NO INICIADA`.)_ **Implementación:** creados `apps/api/src/modules/xml-processing/xml-validation.ts` (función pura `validateXml(xmlText, limits)`), `xml-validation.errors.ts` (clase única `XmlValidationError` con `code` discriminante) y `xml-validation.spec.ts` (70 pruebas). **Función pura, no provider**: sin `@Injectable`, sin `Logger`, sin `console`; `xml-processing.module.ts` **no fue modificado** y sigue vacío. **Validación en dos fases** (Addendum §5.3bis): fase 1, barrera sintáctica `XMLValidator.validate(xmlText)` que garantiza la buena formación; fase 2, `XMLParser.parse(xmlText)` de un solo argumento (nunca la sobrecarga deprecated) con `preserveOrder: true` y las opciones seguras de §5.2 (`ignoreAttributes: false`, `parseTagValue/parseAttributeValue: false`, `processEntities: false`, `removeNSPrefix: false`, `maxNestedTags: limits.maxDepth`, `strictReservedNames: true`), más el **recorrido iterativo con stack** (sin recursión) que aplica profundidad/nodos/atributos con corte inmediato — definiciones en §5.3bis (`I-07`). Orden y número de invocaciones verificados por pruebas: `validate` siempre antes de `parse`, `parse` no se ejecuta si la fase 1 falla, y una sola invocación de cada una por documento válido. **Prototype pollution:** `__proto__`/`constructor`/`prototype` como nombre de tag son rechazados de forma nativa e incondicional (`XML_DANGEROUS_PROPERTY`); como nombre de atributo quedan neutralizados por el prefijo `@_` — ambos comportamientos verificados empíricamente, no asumidos. **Cobertura dirigida:** `xml-validation.ts` 97.5 % statements / 91.89 % branches / 100 % functions / 97.5 % lines; `xml-validation.errors.ts` 100 % en las cuatro dimensiones — las dos ramas no cubiertas son guardas que `noUncheckedIndexedAccess` exige y son inalcanzables, documentadas en el código. **Regresión completa:** 39 suites / 540 pruebas `PASSED`. Correcciones documentales `I-04` a `I-07` aplicadas en Addendum §5.2/§5.3bis/§5.4.
- **Historial de auditoría:** **auditoría `FAILED`** sobre HEAD `f2bb09c1bf8dde487000ede4e0c9a8ca98766df0` (2026-08-01, Codex) con un hallazgo **`ALTO`**: `validateXml` aceptaba tags sin cerrar, tags mal anidados, XML truncado y texto significativo fuera de la raíz, porque `XMLParser.parse()` de un solo argumento **no valida la buena formación** — un XML inválido podía avanzar con contenido truncado o descartado en silencio, **vulnerando BR-XML-001**, y `E5-S3-T05` no tiene asignada esa responsabilidad. **`RESOLVED`** mediante la barrera sintáctica de la fase 1 (`XMLValidator.validate()` antes del parseo), con los cuatro casos convertidos en pruebas de rechazo (`XML_SYNTAX_INVALID`); resolución confirmada por la **reauditoría `PASSED`** sobre HEAD `66703145866349bc362c1f6a1c7bd8cf96ca3466`, registrada como `H-T04-01` en `docs/engineering/audits/E5-S3-T04_FINAL_AUDIT.md`. La redacción previa de esta tarjeta y del Addendum, que describía esa tolerancia como una limitación aceptada del diseño, queda **sustituida** — ya no se tolera ningún XML mal formado. Deuda declarada: `XMLValidator` es `@deprecated` en `5.10.1`; se adopta por ser la única opción que cumple el objetivo literal sin agregar dependencia nueva, con `fast-xml-validator` registrado como ruta de migración (Addendum §5.3bis).

#### E5-S3-T05 — `detectCfdiVersion` (`cfdi-40-extractor.ts`)

- **Objetivo:** detectar la versión del CFDI y rechazar lo no soportado (AD-8: MVP es únicamente CFDI 4.0). **Alcance estrecho, decisión vinculante (corrección `I-08`):** T05 identifica la raíz, resuelve su namespace efectivo, confirma que es un `Comprobante` CFDI y lee `Version` — nunca extrae `UUID`, `RfcEmisor`/`RfcReceptor`, `SubTotal`/`Total`, `Moneda`, `Fecha`, `TipoDeComprobante`, conceptos, impuestos ni el Timbre Fiscal Digital. Esos campos siguen siendo responsabilidad exclusiva de `E5-S3-T06` (encabezado), `E5-S3-T07` (conceptos), `E5-S3-T08` (impuestos) y `E5-S3-T09` (ambiguos) — T05 no las adelanta ni las duplica.
- **Archivos/módulos probables:** `apps/api/src/modules/xml-processing/cfdi-40-extractor.ts` (función pura, sin sufijo `.service` — corrección `I-11`: mismo patrón sin NestJS que `xml-pre-validation.ts`/`xml-validation.ts`, coherente con que ningún consumidor la inyecta todavía).
- **Dependencias:** `E5-S3-T04`.
- **Precondiciones:** XML válido estructuralmente (árbol `preserveOrder` ya devuelto por `validateXml`).
- **Acciones:** 1) Localizar la única raíz de elemento. 2) Resolver el namespace efectivo de la raíz **por URI, nunca por prefijo textual** (§5.3ter — un prefijo distinto de `cfdi` no invalida un CFDI real, y un prefijo `cfdi` no garantiza un namespace correcto). 3) Confirmar `localName = 'Comprobante'`. 4) Confirmar que el namespace resuelto pertenece a la familia CFDI reconocida (`cfd/4` y `cfd/3` — ver corrección `I-12`). 5) Leer `Version` con comparación exacta contra `'4.0'`, sin normalizar formato. 6) `3.3` u otra distinta de `4.0` → `UNSUPPORTED_CFDI_VERSION`. 7) No es un CFDI (raíz o namespace incorrectos) → `CFDI_STRUCTURE_INVALID`.
- **Criterio de aceptación:** CFDI 3.3 nunca se extrae, siempre se rechaza (§17 fuera de alcance, ya reconfirmado). _(Corrección `I-10`, alineada con `H-T03`/`H-T04-01`: el error de T05 es un `CfdiExtractionError` interno con `code` discriminante — nunca "Transacción C + `UnrecoverableError`" directamente; esa traducción externa sigue siendo, sin excepción, responsabilidad exclusiva del worker de Sprint 4, AD-11.)_
- **Pruebas requeridas:** ver `E5-S3-T12`.
- **Riesgos:** identificar CFDI por coincidencia textual de prefijo (`startsWith('cfdi:')`) en vez de por namespace URI resuelto — aceptaría un documento falsificado con prefijo `cfdi:` y namespace inventado, y rechazaría un CFDI legítimo con prefijo alternativo o namespace por defecto.
- **Evidencia de cierre:** diff del extractor.
- **Gate asociado:** ninguno directo.
- **Estado inicial:** `BLOCKED`.
- **Estado actual:** `PASSED` (cierre administrativo 2026-08-01, HEAD auditado `598aebe35f8eb0162158b1e59f6bc560b618019e`, reauditoría final independiente `READ ONLY`: `docs/engineering/audits/E5-S3-T05_FINAL_AUDIT.md`, hallazgo `H-T05-01` `RESOLVED`, sin hallazgos nuevos). _(Estado histórico previo, mismo día: `READY_FOR_AUDIT` tras corregir el hallazgo `MEDIO` — superado por el veredicto `PASSED`. Antes: `READY_FOR_AUDIT` con auditoría `FAILED` pendiente de corrección. Antes: `READY_FOR_AUDIT` (implementado 2026-08-01, pendiente de auditoría independiente `READ ONLY` de Codex) — superado.)_ **Implementación:** creados `apps/api/src/modules/xml-processing/cfdi-40-extractor.ts` (función pura `detectCfdiVersion(parsedXml)`, más las utilidades exportadas `splitQualifiedName`, `extendNamespaceScope` y `resolveNamespacePrefix` para que `E5-S3-T06`–`T09` resuelvan namespaces de sus propios elementos sin reimplementar esta lógica), `cfdi-extraction.errors.ts` (clase única `CfdiExtractionError` con `code` discriminante: `CFDI_STRUCTURE_INVALID` | `UNSUPPORTED_CFDI_VERSION`) y `cfdi-40-extractor.spec.ts` (46 pruebas). **Función pura, no provider**: sin `@Injectable`, sin `Logger`, sin `console`; `xml-processing.module.ts` **no fue modificado** y sigue vacío. **No vuelve a invocar `fast-xml-parser`**: opera exclusivamente sobre el árbol `preserveOrder` ya construido por `E5-S3-T04`, sin volver a validar sintaxis ni a contar profundidad/nodos/atributos. **Identidad por namespace URI, nunca por prefijo** (hallazgo determinante, verificado empíricamente): el prefijo de un elemento XML es arbitrario — un CFDI 4.0 legítimo puede usar `cfdi:`, un prefijo alternativo, o el namespace por defecto sin prefijo; un documento falsificado puede declarar `xmlns:cfdi` apuntando a un namespace inventado. La resolución respeta ámbito y reasignación (`xmlns`/`xmlns:prefijo` heredado y sobreescribible por descendiente), igual que la especificación XML de namespaces. **Namespace de CFDI 3.3 reconocido deliberadamente** (`I-12`): necesario para que un CFDI 3.3 real produzca `UNSUPPORTED_CFDI_VERSION` (AD-8) en vez de `CFDI_STRUCTURE_INVALID` — reconocerlo no lo acepta, solo determina el código de rechazo correcto. **Comparación exacta de `Version`:** solo `"4.0"` se acepta; `"4"`, `"4.00"`, `"4 .0"`, `"v4.0"` se tratan como versión distinta, nunca normalizada. **Hallazgo empírico adicional:** `Version=" 4.0 "` (espacios al inicio/final) no llega distinguible a T05 porque `trimValues: true` de `E5-S3-T04` (inmutable) ya recorta también los valores de atributo, no solo el texto — documentado en el código y en la suite de pruebas, no es una normalización que T05 realice. **Cobertura dirigida:** `cfdi-40-extractor.ts` 97.56 % statements / 93.93 % branches / 100 % functions / 97.53 % lines; `cfdi-extraction.errors.ts` 100 % en las cuatro dimensiones — las dos ramas no cubiertas son guardas de tipos inalcanzables con el árbol real de `fast-xml-parser`, documentadas en el código. **Regresión completa:** 40 suites / 586 pruebas `PASSED`. Correcciones documentales `I-08` a `I-12` aplicadas en esta tarjeta y en Addendum §5.3ter.
- **Historial de auditoría:** **auditoría `FAILED`** sobre HEAD `cd3a26483c4b30968911c7b74481cf112f47e092` (2026-08-01, Codex) con un hallazgo **`MEDIO`**: `resolveNamespacePrefix` consultaba `prefix in actual.bindings`, que también encuentra propiedades **heredadas** de `Object.prototype` — si un proceso contaminaba globalmente `Object.prototype.cfdi` con el URI oficial de CFDI 4.0, `<cfdi:Comprobante Version="4.0"/>` **sin** `xmlns:cfdi` declarado resolvía igual que un binding real, incumpliendo el rechazo obligatorio de prefijos no declarados. **`RESOLVED`** mediante: (a) `extendNamespaceScope` ahora construye `bindings` con `Object.create(null)` (sin prototipo); (b) `resolveNamespacePrefix` consulta únicamente propiedad propia vía `Object.hasOwn(actual.bindings, prefix)`, nunca el operador `in`. Contrato público (`CfdiRootElement`, códigos de error) **sin cambios**. Se agregaron 4 pruebas nuevas: una de contaminación real de extremo a extremo (`Object.prototype.cfdi` = URI oficial, árbol T04→T05 real de un `Comprobante` sin `xmlns:cfdi`, espera `CFDI_STRUCTURE_INVALID`, restauración garantizada en `afterEach`), una equivalente sobre `resolveNamespacePrefix` directamente, y dos unitarias contrastando propiedad heredada (nunca cuenta) contra propiedad propia (sí cuenta) incluso cuando el prototipo declara la misma clave. Regresión de namespace confirmada sin cambios: ámbito padre, reasignación local, namespace por defecto, prefijo alternativo y prefijo no declarado siguen pasando con las pruebas ya existentes. **Cobertura dirigida sin regresión:** `cfdi-40-extractor.ts` 97.56 % statements / 93.93 % branches / 100 % functions / 97.53 % lines (idéntica a la implementación original — la corrección no introdujo ni eliminó ramas alcanzables). **Regresión completa:** 40 suites / 590 pruebas `PASSED` (586 + 4 nuevas). Corrección verificada en el commit `598aebe`. La **reauditoría `PASSED`** sobre ese mismo HEAD `598aebe35f8eb0162158b1e59f6bc560b618019e` confirmó el hallazgo `H-T05-01` `RESOLVED`, sin hallazgos nuevos, registrada en `docs/engineering/audits/E5-S3-T05_FINAL_AUDIT.md`. `E5-S3-T05` queda **`PASSED`**; `E5-S3-T06` queda habilitada, no iniciada.
...
- **Acciones:** 1) `folioFiscal` (Timbre Fiscal Digital, UUID de 36 caracteres). 2) `rfcEmisor`, `rfcReceptor`, `issuedAtLocal` (string exacto `AAAA-MM-DDThh:mm:ss`, hora local sin offset, nunca `Date` — D-009), `subtotal`, `total` (cadena decimal exacta, nunca `Float` — BR-GLB-004), `currency`, `tipoComprobante`.
...
- **Gate asociado:** ninguno directo.
- **Estado inicial:** `BLOCKED`.
- **Estado actual:** **`PASSED`**. Auditoría final independiente `READ ONLY` sobre el snapshot `84babc183e84885f56284de5142ee70d72d3cf84`, implementación `3de9a6cb8744439326da1dbe8ce51e2f3f00b24a`; evidencia: [`E5-S3-T06_FINAL_AUDIT.md`](audits/E5-S3-T06_FINAL_AUDIT.md). Confirmados los ocho campos, TFD por URI efectivo, UUID RFC 4122 de 36 caracteres, `issuedAtLocal: string` sin `Date`/UTC y `cfdi-extraction.errors.ts` intacto. Observación `BAJO` no bloqueante: falso negativo CRLF de infraestructura de pruebas; `069a776` endureció posteriormente esas pruebas sin modificar producción de T06. _(Estado histórico previo: `IMPLEMENTADA · PENDIENTE DE AUDITORÍA FINAL`.)_ El análisis técnico previo (2026-08-02) había detectado dos bloqueantes, ambos resueltos antes de esta implementación:
  - **`I-15` — `RESOLVED`.** El namespace oficial del Timbre Fiscal Digital 1.1 (`http://www.sat.gob.mx/TimbreFiscalDigital`) solo existía en _fixtures_ de pruebas, nunca como constante normativa. Queda registrado como constante arquitectónica vinculante en Addendum §5.3quater, junto con las reglas de resolución por URI efectivo (nunca por el prefijo `tfd`), la localización del Timbre únicamente dentro de `Complemento`, y el `UUID` requerido de longitud exacta 36 (RFC 4122), nunca registrado en logs ni errores.
  - **`I-14` — `RESOLVED` e implementado el 2026-08-04 (D-009 `PASSED`).** El atributo `Fecha` de CFDI 4.0 es hora **local sin offset** (`AAAA-MM-DDThh:mm:ss`), por lo que el parser tiene prohibido derivar un instante: `new Date(fecha)` produce un resultado dependiente de la zona horaria del host (verificado empíricamente: `2026-07-15T10:30:00` → `16:30:00Z` en `America/Mexico_City`), lo que rompe la extracción determinista de AD-10.1 y exigiría asumir una zona implícita que `docs/08_API_DESIGN.md` ya prohíbe. **Contrato aplicado:** `ExtractedCfdiAggregate.issuedAtLocal: string` (sustituye a `issuedAt: Date`), persistido textualmente como `issuedAtLocal String @map("issued_at") @db.VarChar(19)` mediante la migración correctiva nueva `20260804013104_preserve_cfdi_local_issue_datetime`, verificando por consulta directa que la tabla `cfdis` estaba vacía (`COUNT(*) = 0`) antes de generarla y antes de aplicarla. Esta corrección modificó artefactos `PASSED` de Sprint 1 y Sprint 2 (`E5-S2-T01`/`T02`/`T06`/`T10`, `E5-S1-T05`/`T07`/`T09`), y su reauditoría final independiente confirmó la ausencia de regresiones en atomicidad y persistencia. Detalle completo en Addendum §5.3quater, `brain/DECISIONS.md` D-009 y `docs/engineering/audits/D-009_FINAL_AUDIT.md`.
  - **Cierre:** `E5-S3-T06` queda `PASSED`. Su cierre individual no adelantó `E5-S3-T07`; en esta consolidación T07 se cierra mediante su propia auditoría. Sprint 3 continúa `IN_PROGRESS` y `E5-S3-T08`–`T12` conservan su estado.
...
- **Estado inicial:** `BLOCKED`.
- **Estado actual:** **`PASSED`** (cierre administrativo 2026-08-07; auditoría final independiente `READ ONLY`: [`E5-S3-T07_FINAL_AUDIT.md`](audits/E5-S3-T07_FINAL_AUDIT.md), sobre `1abb93d1c9fa7322dc5d4f4f7b4a05b713b3ac86`, `069a776b5d1aebfdb01a9875920a059bc0725ac3` y `f74f65e218578516eceaabd75202c320a4ca41cc`). Confirmados `position = index + 1`, rechazo completo ante concepto obligatorio inválido y `taxes: []` incondicional. El hallazgo previo sobre `ambiguousFields` fue un falso positivo normativo: las coincidencias son JSDoc, no código ejecutable. Este cierre no modifica el estado de otras tareas.
...
- **Estado inicial:** `BLOCKED`.
- **Estado actual:** `PASSED`. Implementación inicial integrada mediante `a8a700b` (junto con `E5-S4-T09`); el endurecimiento de una respuesta exitosa sin `Body` está integrado en main mediante `7eeefda014974215aecbcca482efcc6d922eda68`, cherry-pick equivalente de `889f151ff92b0a1885be7ec55a274572a6c7e6df`. La auditoría final independiente `READ ONLY` sobre `889f151` confirma `GetObjectCommand`, `transformToByteArray()`, `DisabledStorageAdapter`, la clasificación de 404/403/red y el rechazo controlado de `Body` ausente o inutilizable. Evidencia persistente: [`E5-S4-T01_FINAL_AUDIT.md`](audits/E5-S4-T01_FINAL_AUDIT.md). Sin `XmlExtractionProcessor`, `@Processor` ni `WorkerHost`; el consumo del método sigue reservado a `E5-S4-T02`. Sin cambios de Prisma ni migraciones.
...
- **Estado inicial:** `BLOCKED`.
- **Estado actual:** `PASSED`. Código integrado mediante `a8a700b` (junto con `E5-S4-T01`) y auditado independientemente sobre `889f151ff92b0a1885be7ec55a274572a6c7e6df`; `7eeefda` endurece exclusivamente T01 y no modifica T09. Las 14 variables canónicas de Addendum §10.3 existen en `jobsEnvSchema`/`xmlEnvSchema`, se fusionan en `packages/config/src/server.ts` y fallan al arranque ante valores fuera de rango; las ausentes usan el default MVP. `JOBS_ATTEMPTS` y `JOBS_BACKOFF_DELAY_MS` se inyectan desde `SERVER_CONFIG` en `BullMqJobsQueueAdapter`; el backoff canónico es `5000 ms`. Las variables de reconciliación, umbrales de atasco y retención permanecen validadas y disponibles pero deliberadamente sin cablear. Evidencia persistente: [`E5-S4-T09_FINAL_AUDIT.md`](audits/E5-S4-T09_FINAL_AUDIT.md). La observación de que `jobs.repository.spec.ts` y `jobs.service.spec.ts` requieren PostgreSQL dedicado no es defecto de código. Sin `XmlExtractionProcessor`, `WorkerHost`, `@Processor`, Prisma ni migraciones.
...
- **Sprint 3** — **`IN_PROGRESS`.** `E5-S3-T01` → **`PASSED`** el 2026-07-31 (cierre administrativo tras auditoría final independiente `READ ONLY`, Codex, sin hallazgos: `docs/engineering/audits/E5-S3-T01_FINAL_AUDIT.md`): creado `apps/api/src/modules/xml-processing/xml-processing.module.ts` — módulo NestJS vacío (`imports: []`, `providers: []`, `exports: []`), sin `forwardRef()`, sin dependencias con `JobsModule`/`DocumentsModule`/`StorageModule`/`@contaia/database`/`bullmq`/`fast-xml-parser`. `E5-S3-T02` (instalación de `fast-xml-parser` en versión exacta `5.10.1`, sin configuración ni lógica de parseo) → **`PASSED`** el 2026-07-31 (cierre administrativo tras reauditoría final independiente `READ ONLY`, Codex, HEAD `939c88943225f2e45ee1de466294ad1725b7de3a`, sin hallazgos nuevos: `docs/engineering/audits/E5-S3-T02_FINAL_AUDIT.md`); una auditoría previa sobre HEAD `fa71efb52ac995bdb4105b3425b225692600ac0f` había resultado `FAILED` con dos hallazgos `MEDIO` (`H-T02-01` CVE `GHSA-gh4j-gqv2-49f6` en la `4.5.7` originalmente instalada; `H-T02-02` desincronización de Addendum §5.2), ambos `RESOLVED` mediante actualización a `5.10.1` fijada y sincronización de §5.2. `E5-S3-T03` (prevalidaciones de seguridad sobre el Buffer XML — función pura `preValidateXmlBuffer`, sin `fast-xml-parser` y sin providers NestJS; 64 pruebas unitarias, cobertura dirigida ≥ 95 % en las cuatro dimensiones) → **`PASSED`** el 2026-08-01 (cierre administrativo tras auditoría final independiente `READ ONLY`, Codex, HEAD `79ce98489ae9300193d1380480278840f3563a1f`, hallazgo `H-T03` `RESOLVED`, sin hallazgos nuevos: `docs/engineering/audits/E5-S3-T03_FINAL_AUDIT.md`). `E5-S3-T04` (validación estructural del XML — función pura `validateXml` en dos fases: barrera sintáctica `XMLValidator.validate()` seguida de `XMLParser.parse()` y recorrido iterativo con stack para profundidad/nodos/atributos; 70 pruebas unitarias, cobertura dirigida ≥ 90 % en las cuatro dimensiones) → **`PASSED`** el 2026-08-01 (cierre administrativo tras reauditoría final independiente `READ ONLY`, Codex, HEAD `66703145866349bc362c1f6a1c7bd8cf96ca3466`, hallazgo `H-T04-01` `RESOLVED`, sin hallazgos nuevos: `docs/engineering/audits/E5-S3-T04_FINAL_AUDIT.md`); una auditoría previa sobre HEAD `f2bb09c1bf8dde487000ede4e0c9a8ca98766df0` había resultado **`FAILED`** con un hallazgo **`ALTO`** (se aceptaban tags sin cerrar, mal anidados, XML truncado y texto significativo fuera de la raíz, vulnerando BR-XML-001), `RESOLVED` mediante la barrera sintáctica previa al parseo. Correcciones documentales `I-04` a `I-07` aplicadas en Addendum §5.2/§5.3bis/§5.4. `E5-S3-T05` (detección de CFDI 4.0 — función pura `detectCfdiVersion`, identidad por namespace URI nunca por prefijo, comparación exacta de `Version`; 46 pruebas unitarias, cobertura dirigida ≥ 90 % en las cuatro dimensiones) implementada el 2026-08-01; una auditoría sobre HEAD `cd3a26483c4b30968911c7b74481cf112f47e092` resultó **`FAILED`** con un hallazgo **`MEDIO`** (`resolveNamespacePrefix` usaba `prefix in bindings`, que también encuentra propiedades heredadas de `Object.prototype` — una contaminación global de `Object.prototype.cfdi` con el URI oficial habría hecho que un prefijo `cfdi` nunca declarado en el XML resolviera igual que uno legítimo), **`RESOLVED`** mediante `bindings` sin prototipo (`Object.create(null)`) y comprobación de propiedad propia (`Object.hasOwn`), con 4 pruebas nuevas de contaminación/herencia agregadas; contrato público y clasificación de errores sin cambios; 40 suites / 590 pruebas `PASSED` en la regresión completa; → **`PASSED`** el 2026-08-01 (cierre administrativo tras reauditoría final independiente `READ ONLY`, Codex, sobre el mismo HEAD `598aebe35f8eb0162158b1e59f6bc560b618019e`, hallazgo `H-T05-01` `RESOLVED`, sin hallazgos nuevos: `docs/engineering/audits/E5-S3-T05_FINAL_AUDIT.md`). Correcciones documentales `I-08` a `I-12` aplicadas en esta tarjeta y en Addendum §5.3ter. `E5-S3-T06` queda **`HABILITADA`** y **no iniciada**, pero **bloqueada por corrección de contrato** desde el 2026-08-02: su análisis técnico previo detectó dos bloqueantes, `I-15` (namespace oficial del Timbre Fiscal Digital sin registro normativo) → **`RESUELTO`** en Addendum §5.3quater, e `I-14` (el atributo `Fecha` es hora local sin offset y no puede convertirse a `Date` sin depender de la zona horaria del host) → **decisión `RESUELTA`** el 2026-08-02 con D-009 `APROBADA` (contrato `ExtractedCfdiAggregate.issuedAtLocal: string`, persistido como `String @map("issued_at") @db.VarChar(19)` mediante migración correctiva nueva), e **implementada el 2026-08-04**: verificado por consulta directa que la tabla `cfdis` estaba vacía (`COUNT(*) = 0`) antes de generar y de aplicar la migración correctiva nueva `20260804013104_preserve_cfdi_local_issue_datetime`, de modo que ningún instante fue convertido y ninguna zona horaria fue asumida; modificó artefactos `PASSED` de Sprint 1 y Sprint 2, por lo que quedó `READY_FOR_AUDIT` hasta su cierre por auditoría final independiente `READ ONLY` el 2026-08-03 (Codex, HEAD `6934b26f6f568c27d697f8c3f31cebaddb0261a0`, sin hallazgos nuevos: `docs/engineering/audits/D-009_FINAL_AUDIT.md`) → **D-009 `PASSED`**; `I-14` e `I-15` `RESOLVED`. `E5-S3-T06` sigue habilitada y no iniciada, ya no bloqueada por auditoría pendiente. `E5-S3-T07`–`T12` permanecen `BLOCKED`. Siguiente paso: implementación controlada de `E5-S3-T06` con Claude Code, usando el análisis técnico ya aprobado y el contrato `issuedAtLocal` vigente. _(Estado histórico previo, hasta el 2026-08-03: `READY_FOR_AUDIT`, pendiente de reauditoría independiente — superado por el veredicto `PASSED` registrado arriba. Antes, mismo día 2026-08-02: `READY_FOR_AUDIT` tras corregir el hallazgo `MEDIO`, pendiente de reauditoría. Antes: `READY_FOR_AUDIT` con auditoría `FAILED` (hallazgo `MEDIO`) pendiente de corrección. Antes: `READY_FOR_AUDIT` — implementado el 2026-07-31, pendiente de auditoría independiente — superado.)_ _(Sobre `E5-S3-T02`, estado histórico previo, mismo día: `READY_FOR_AUDIT` tras corrección de auditoría, pendiente de reauditoría — superado por el veredicto `PASSED` registrado arriba. Antes: `READY_FOR_AUDIT` con auditoría `FAILED` pendiente de corrección. Antes: `NOT_STARTED`.)_
