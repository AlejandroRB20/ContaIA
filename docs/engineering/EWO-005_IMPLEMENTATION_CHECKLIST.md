...
- **Estado inicial:** `BLOCKED`.
- **Estado actual:** `READY_FOR_AUDIT` (implementado 2026-08-01, pendiente de auditoría independiente `READ ONLY` de Codex). **Implementación:** creados `apps/api/src/modules/xml-processing/xml-validation.ts` (función pura `validateXml(xmlText, limits)`), `xml-validation.errors.ts` (clase única `XmlValidationError` con `code` discriminante) y `xml-validation.spec.ts` (62 pruebas). **Función pura, no provider**: sin `@Injectable`, sin `Logger`, sin `console`; `xml-processing.module.ts` **no fue modificado** y sigue vacío. **Parseo único** (`XMLParser.parse(xmlText)` de un solo argumento — nunca `XMLValidator`, nunca `fast-xml-validator`, verificado por prueba de instrumentación) con `preserveOrder: true` y las opciones seguras de Addendum §5.2/§5.3bis (`ignoreAttributes: false`, `parseTagValue/parseAttributeValue: false`, `processEntities: false`, `removeNSPrefix: false`, `maxNestedTags: limits.maxDepth`, `strictReservedNames: true`). **Recorrido iterativo con stack** (sin recursión) sobre el árbol ya parseado, con corte inmediato al superar profundidad/nodos/atributos — definiciones registradas en Addendum §5.3bis (`I-07`). **Hallazgo empírico determinante** (verificado contra la versión instalada, no supuesto): `XMLParser.parse()` de un solo argumento no es un validador de buena formación — tolera en silencio tags sin cerrar, tags mal anidados, XML truncado y texto fuera de la raíz, documentado explícitamente en el código y en la suite de pruebas, no oculto. **Prototype pollution:** `__proto__`/`constructor`/`prototype` como nombre de tag son rechazados de forma nativa e incondicional por la librería (`XML_DANGEROUS_PROPERTY`); como nombre de atributo quedan neutralizados por el prefijo `@_` sin necesitar rechazo adicional — ambos comportamientos verificados empíricamente, no asumidos. **Cobertura dirigida:** `xml-validation.ts` 97.33 % statements / 91.66 % branches / 100 % functions / 97.33 % lines; `xml-validation.errors.ts` 100 % en las cuatro dimensiones — las dos ramas no cubiertas son guardas de tipos inalcanzables con la librería real, documentadas en el código. **Regresión completa:** 39 suites / 532 pruebas `PASSED`. Correcciones documentales `I-04` a `I-07` aplicadas en Addendum §5.2/§5.3bis/§5.4. Pendiente: auditoría independiente `READ ONLY` de Codex antes de marcar `PASSED`.
