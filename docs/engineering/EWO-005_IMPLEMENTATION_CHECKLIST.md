...
- **Gate asociado:** ninguno directo.
- **Estado inicial:** `BLOCKED`.
- **Estado actual:** `IMPLEMENTADA · PENDIENTE DE AUDITORÍA FINAL`. Integrada en `feature/frontend-ux-audit` el 2026-08-06 (commit `3de9a6cb8744439326da1dbe8ce51e2f3f00b24a`, cherry-pick limpio sin conflictos desde el commit aislado `a655bb94a4535e1f9457f3a127a7864b75ecb61a` construido en worktree separado sobre la misma base `b5b289d32fdcc8d7ab61fd62ecfe0316b8c75be8`). **Archivo real:** la extracción de encabezado se agregó al mismo archivo de `E5-S3-T05`, `apps/api/src/modules/xml-processing/cfdi-40-extractor.ts` (no se creó `cfdi-40-extractor.service.ts` — sigue siendo función pura, no provider), junto con `cfdi-40-extractor.spec.ts`. **Pruebas:** 78 en el archivo (50 preexistentes de `E5-S3-T05` + 28 nuevas de `E5-S3-T06`), 78/78 `PASSED` tras la integración; regresión completa de `apps/api` 41 suites / 642 pruebas `PASSED`, sin regresiones. **Contrato `D-009` preservado:** `issuedAtLocal: string`, sin `new Date()`, sin `Date.parse()`, sin sufijo `Z` ni conversión UTC — verificado en el código integrado y en pruebas dedicadas que rechazan formas inválidas sin convertir a `Date`. **Alcance verificado sin extensión:** no incluye extracción de `concepts[]`, `cfdiTaxes[]` ni `ambiguousFields[]` (`E5-S3-T07`–`T09`, siguen `BLOCKED`), no modifica `cfdi-extraction.errors.ts`, no toca Prisma/migraciones/frontend/arquitectura de worker. **Pendiente:** auditoría final independiente `READ ONLY` de Codex — esta integración no constituye cierre `PASSED`. _(Estado histórico previo, hasta el 2026-08-06: `HABILITADA` / `NO INICIADA` — "No existe ningún archivo propio de `E5-S3-T06`" fue cierto hasta esa fecha; superado por la integración registrada arriba.)_ El análisis técnico previo (2026-08-02) había detectado dos bloqueantes, ambos resueltos antes de esta implementación:
  ...
  - **Siguiente paso:** auditoría final independiente `READ ONLY` de Codex sobre el commit `3de9a6cb8744439326da1dbe8ce51e2f3f00b24a`. `E5-S3-T07`–`T12` conservan su estado vigente (`BLOCKED`). Sprint 3 continúa `IN_PROGRESS` — no se marca completado.
