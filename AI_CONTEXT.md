...
| D-011 | `cfdi.read` para Auditor y Supervisor; `document.download` separado   | **IMPLEMENTADA · PENDIENTE DE REAUDITORÍA** (ratificada 2026-08-06) — `cfdi.read` concedido a Auditor/Supervisor; `document.download` como clave independiente para Administrador/Contador/Auxiliar/Supervisor/Auditor. `API-0026` debe exigirla: `document.read` y `cfdi.read` nunca autorizan la descarga del binario |
| D-012 | Identidad canónica de CFDI mediante `documentId`                      | **IMPLEMENTADA · PENDIENTE DE REAUDITORÍA** (2026-08-04) — `T04` sincronizó `docs/14`, `docs/31`, `docs/32` y `EWO-005_DOCUMENTS_FISCAL_PLAN.md` a `/{companyId}/documentos/{documentId}/cfdi`                                                                                                                          |
